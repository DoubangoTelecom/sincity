...
@interface SCVideoProducerIOS : NSObject
#if SC_PRODUCER_IOS_HAVE_VIDEO_CAPTURE
...
#endif
{
    ...
    int mFps;
#if SC_PRODUCER_IOS_HAVE_VIDEO_CAPTURE
    const struct sc_producer_video_ios_s* mWrappedProducer; // Not owner, do not release
    UIView* mPreview;
...
-(SCVideoProducerIOS*) initWithWrappedProducer:(const struct sc_producer_video_ios_s*)wrappedProducer;
...
#if SC_PRODUCER_IOS_HAVE_VIDEO_CAPTURE
@interface SCVideoProducerIOS (VideoCapture)
...
typedef struct sc_producer_video_ios_s
{
    ...
    SCVideoProducerIOS* p_producer;
    
    TSK_DECLARE_SAFEOBJ;
}
sc_producer_video_ios_t;
...
static int _sc_producer_video_ios_set(tmedia_producer_t *p_self, const tmedia_param_t* pc_param)
{
    sc_producer_video_ios_t* p_ios = (sc_producer_video_ios_t*)p_self;
    if (!p_self || !pc_param) {
        SC_DEBUG_ERROR_EX(kSCMobuleNameiOSVideoProducer, "Invalid parameter");
        return -1;
    }
    ...
    if (pc_param->value_type == tmedia_pvt_int64) {
        if (tsk_striequals(pc_param->key, "local-hwnd") || tsk_striequals(pc_param->key, "preview-hwnd")) {
            ...
            if (view) {
                SC_ASSERT([view isKindOfClass:[UIView class]]);
            }
    ...
    SC_DEBUG_ERROR_EX(kSCMobuleNameiOSVideoProducer, "set(name=%s, value_type=%d) not supported", pc_param->key, pc_param->value_type);
    return -2;
}

static int _sc_producer_video_ios_prepare(tmedia_producer_t* p_self, const tmedia_codec_t* pc_codec)
{
    sc_producer_video_ios_t* p_ios = (sc_producer_video_ios_t*)p_self;
    int ret = 0;
    ...
    if (p_ios->b_prepared) {
        SC_DEBUG_INFO_EX(kSCMobuleNameiOSVideoProducer, "Already prepared");
        goto bail;
    }
...
static int _sc_producer_video_ios_start(tmedia_producer_t* p_self)
{
    sc_producer_video_ios_t* p_ios = (sc_producer_video_ios_t*)p_self;
    int ret = 0;
    NSAutoreleasePool *pool = nil;
    ...
    if (p_ios->b_started) {
        SC_DEBUG_ERROR_EX(kSCMobuleNameiOSVideoProducer, "Already started");
        goto bail;
    }
    
    if (!p_ios->b_prepared) {
        SC_DEBUG_ERROR_EX(kSCMobuleNameiOSVideoProducer, "Not prepared");
        ret = -1;
        goto bail;
    }
...
static int _sc_producer_video_ios_pause(tmedia_producer_t* p_self)
{
    sc_producer_video_ios_t* p_ios = (sc_producer_video_ios_t*)p_self;
    int ret = 0;
    NSAutoreleasePool *pool = nil;
    ...
    if (p_ios->b_paused) {
        SC_DEBUG_INFO_EX(kSCMobuleNameiOSVideoProducer, "Already paused");
        goto bail;
    }
#if SC_PRODUCER_IOS_HAVE_VIDEO_CAPTURE
    pool = [[NSAutoreleasePool alloc] init];
...
static int _sc_producer_video_ios_stop(tmedia_producer_t* p_self)
{
    sc_producer_video_ios_t* p_ios = (sc_producer_video_ios_t*)p_self;
    int ret = 0;
    NSAutoreleasePool *pool = nil;
...
//
/* constructor */
static tsk_object_t* _sc_producer_video_ios_ctor(tsk_object_t *self, va_list * app)
{
    sc_producer_video_ios_t *p_ios = (sc_producer_video_ios_t *)self;
    if (p_ios) {
        /* init base */
        tmedia_producer_init(TMEDIA_PRODUCER(p_ios));
        /* init self */
        p_ios->p_producer = [[SCVideoProducerIOS alloc] initWithWrappedProducer:p_ios];
        if (!p_ios->p_producer) {
            SC_DEBUG_ERROR_EX(kSCMobuleNameiOSVideoProducer, "Failed to create proxy video producer");
            return tsk_null;
        }
...
/* destructor */
static tsk_object_t* _sc_producer_video_ios_dtor(tsk_object_t * self)
{
    sc_producer_video_ios_t *p_ios = (sc_producer_video_ios_t *)self;
    if (p_ios) {
        /* stop */
        if (p_ios->b_started) {
            _sc_producer_video_ios_stop((tmedia_producer_t*)p_ios);
        }
        ...
        SC_DEBUG_INFO_EX(kSCMobuleNameiOSVideoProducer, "*** destroyed ***");
    }
    
    return self;
}
/* object definition */
static const tsk_object_def_t sc_producer_video_ios_def_s =
{
    sizeof(sc_producer_video_ios_t),
    _sc_producer_video_ios_ctor,
    _sc_producer_video_ios_dtor,
    tsk_null,
};
/* plugin definition*/
static const tmedia_producer_plugin_def_t sc_producer_video_ios_plugin_def_s =
{
    &sc_producer_video_ios_def_s,
    tmedia_video,
    "iOS video producer",
    
    _sc_producer_video_ios_set,
    _sc_producer_video_ios_prepare,
    _sc_producer_video_ios_start,
    _sc_producer_video_ios_pause,
    _sc_producer_video_ios_stop
};
const tmedia_producer_plugin_def_t *sc_producer_video_ios_plugin_def_t = &sc_producer_video_ios_plugin_def_s;



/************************************************
 @implementation SCVideoProducerIOS
 ************************************************/

@implementation SCVideoProducerIOS

-(SCVideoProducerIOS*) initWithWrappedProducer:(const struct sc_producer_video_ios_s*)wrappedProducer {
    SC_ASSERT(wrappedProducer != NULL);
    [self init];
    mStarted = NO;
    mMuted = NO;
    mPaused = NO;
    ...
    mUseFrontCamera = YES;
    mFirstFrame = YES;
    mOrientation = [SCVideoProducerIOS deviceCurrentOrientation:AVCaptureVideoOrientationPortrait];
    mBlankPacketsSent = 0;
...
-(int) prepareWithWidth:(int) width andHeight: (int)height andFps: (int) fps{
    SC_DEBUG_INFO_EX(kSCMobuleNameiOSVideoProducer, "prepareWithWidth(%i,%i,%i)", width, height, fps);
    mWidth = width;
    mHeight = height;
    mFps = fps;
    mPrepared = YES;
...
-(int) start {
    SC_DEBUG_INFO_EX(kSCMobuleNameiOSVideoProducer, "start");
    mStarted = YES;
    mPaused = NO;
    ...
    // set orientation
    [self setOrientation:[SCVideoProducerIOS deviceCurrentOrientation:mOrientation]];
...
-(int) pause{
    SC_DEBUG_INFO_EX(kSCMobuleNameiOSVideoProducer, "pause");
    mPaused = YES;
    return 0;
}

-(int) stop{
    SC_DEBUG_INFO_EX(kSCMobuleNameiOSVideoProducer, "stop");
    mStarted = NO;
...
-(void)dealloc {
    ...
    SC_DEBUG_INFO_EX(kSCMobuleNameiOSVideoProducer, "*** dealloc ***");
}

@end /* @implementation SCVideoProducerIOS */


/************************************************
 @implementation SCVideoProducerIOS(VideoCapture)
 ************************************************/
#if SC_PRODUCER_IOS_HAVE_VIDEO_CAPTURE
@implementation SCVideoProducerIOS(VideoCapture)

- (BOOL)startVideoCapture{
    SC_DEBUG_INFO_EX(kSCMobuleNameiOSVideoProducer, "Starting Video stream");
    if (mCaptureDevice || mCaptureSession) {
        SC_DEBUG_INFO_EX(kSCMobuleNameiOSVideoProducer, "Already capturing");
        return YES;
    }
    
    mCaptureDevice = mUseFrontCamera ? [[SCCameraIOS frontFacingCamera] retain] : [[SCCameraIOS backCamera] retain];
    if (!mCaptureDevice) {
        SC_DEBUG_ERROR_EX(kSCMobuleNameiOSVideoProducer, "Failed to get valide capture device");
        return NO;
    }
    ...
    if (!videoInput){
        SC_DEBUG_ERROR_EX(kSCMobuleNameiOSVideoProducer, "Failed to get video input: %s", error.localizedDescription.UTF8String);
        [mCaptureDevice release];
        mCaptureDevice = nil;
        return NO;
    }
    ...
    dispatch_queue_t queue = dispatch_queue_create("org.doubango.sincity.producer.video.captureoutput", NULL);
    ...
    if ([mCaptureDevice supportsAVCaptureSessionPreset:sessionPreset]) {
        SC_DEBUG_INFO_EX(kSCMobuleNameiOSVideoProducer, "For output video(%dx%d) we selected %s preset", mWidth, mHeight, [sessionPreset UTF8String]);
        mCaptureSession.sessionPreset = sessionPreset;
    }
    else {
        SC_DEBUG_ERROR_EX(kSCMobuleNameiOSVideoProducer, "%s not supported as preset, fallback to %s", [sessionPreset UTF8String], [AVCaptureSessionPresetMedium UTF8String]);
    ...
    if ([[[UIDevice currentDevice] systemVersion] floatValue] < 5) {
        ...
        if (error == nil) {
            ...
            for (AVFrameRateRange * range in [mCaptureDevice activeFormat].videoSupportedFrameRateRanges) {
                SC_DEBUG_INFO_EX(kSCMobuleNameiOSVideoProducer, "videoSupportedFrameRateRanges=[%f - %f]", range.minFrameRate, range.maxFrameRate);
        ...
        best_fps_found:
            SC_DEBUG_INFO_EX(kSCMobuleNameiOSVideoProducer, "requested fps=%d, best fps=%f", mFps, bestFps);
            @try {
            ...
            @catch(NSException * e) {
                SC_DEBUG_ERROR_EX(kSCMobuleNameiOSVideoProducer, "Failed to set frame rate: %s", error.localizedDescription.UTF8String);
            }
        }
        else {
            SC_DEBUG_ERROR_EX(kSCMobuleNameiOSVideoProducer, "lockForConfiguration failed: %s", error.localizedDescription.UTF8String);
        }
    ...
    SC_DEBUG_INFO_EX(kSCMobuleNameiOSVideoProducer, "Video capture started");
    return YES;
}
...
- (BOOL)stopVideoCapture{
    if (mCaptureSession) {
        ...
        SC_DEBUG_INFO_EX(kSCMobuleNameiOSVideoProducer, "Video capture stopped");
    }
    ...
    if ((_producer = (tmedia_producer_t *)tsk_object_ref(TSK_OBJECT(mWrappedProducer)))) {
        ...
        switch (_producer->video.chroma) {
            ...
            case tmedia_chroma_rgb32:
                ...
                break;
            default:
                SC_DEBUG_ERROR_EX(kSCMobuleNameiOSVideoProducer, "Invalid chroma");
                break;
        }
        if (buffer_size > 0 && buffer_size < sizeof(kBlankPacketBuffer)) {
            SC_DEBUG_INFO_EX(kSCMobuleNameiOSVideoProducer, "Sending Blank packet number %d", mBlankPacketsSent);
            ...
            SC_DEBUG_ERROR_EX(kSCMobuleNameiOSVideoProducer, "Invalid buffer size:%f", buffer_size);
        }
    ...
    if(CVPixelBufferLockBaseAddress(pixelBuffer, 0) == kCVReturnSuccess){
        ...
        if (mFirstFrame && producer) { // Hope size and chroma will never change
            ...
            if ([mCaptureDevice respondsToSelector:@selector(activeVideoMaxFrameDuration)]) {
                ...
                SC_DEBUG_INFO_EX(kSCMobuleNameiOSVideoProducer, "Capture fps=%f", fps);
            ...
            switch (pixelFormat) {
                case kCVPixelFormatType_420YpCbCr8BiPlanarVideoRange:
                    ...
                    SC_DEBUG_INFO_EX(kSCMobuleNameiOSVideoProducer, "Capture pixel format=NV12");
                    break;
                case kCVPixelFormatType_422YpCbCr8:
                    producer->video.chroma = tmedia_chroma_uyvy422; // iPhone 3
                    SC_DEBUG_INFO_EX(kSCMobuleNameiOSVideoProducer, "Capture pixel format=UYUY422");
                    break;
                default:
                    producer->video.chroma = tmedia_chroma_rgb32;
                    SC_DEBUG_INFO_EX(kSCMobuleNameiOSVideoProducer, "Capture pixel format=RGB32");
                    break;
            }
            mFirstFrame = NO;
        }
...
@end /* @implementation SCVideoProducerIOS(VideoCapture) */
