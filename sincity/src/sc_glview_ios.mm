...
@implementation SCGlviewIOS {
    ...
    SCAnnotationType _annoCurrentType;
    NSObject<SCAnnotation>* _annoCurrent;
}
...
- (void)initialize {
    ...
    _annoCurrentType = SCAnnotationTypeNone;
    _annoCurrent = nil;
...
-(void)startAnimation {
    @synchronized(self) {
        if (!_animating) {
            ...
            if (_freezed) {
                SC_DEBUG_INFO_EX(kTAG,"/!\\start animation called but _freezed=YES. Video won't be animated");
            }
        }
    }
}
...
-(void)setPAR:(int)numerator denominator:(int)denominator {
    @synchronized(self) {
        if (_parNumerator <= 0 || _parDenominator <= 0) {
            SC_DEBUG_ERROR_EX(kTAG, "Invalid PAR:%d/%d", numerator, denominator);
            assert(0);
        }
...
-(void)setAnnotationType:(SCAnnotationType)annotationType {
    @synchronized(self) {
        _annoCurrentType = annotationType;
    }
}
...
-(void)clearAnnotations {
    ...
    again_layer:
        for (CALayer *layer in [[self layer] sublayers]) {
            if ([layer.name isEqualToString:@"SCAnnotationCircle"] || [layer.name isEqualToString:@"SCAnnotationFreehand"] || [layer.name isEqualToString:@"SCAnnotationArrow"]) {
                [layer removeFromSuperlayer];
                goto again_layer;
            }
        }
        [_annoCurrent release], _annoCurrent = nil;
    }
again_view:
    for (UIView* view in [self subviews]) {
        if (view.tag == kAnnotationTextFieldTag && [view isKindOfClass:[UITextField class]]) {
...
- (GLuint)compileShader:(const char*)shaderSource withType:(GLenum)shaderType {
    ...
    if (compileSuccess == GL_FALSE) {
        ...
        SC_DEBUG_ERROR_EX(kTAG, "%s", [messageString UTF8String]);
        assert(0);
    }
...
- (void)setupContext {
    ...
    if (!_context) {
        SC_DEBUG_ERROR_EX(kTAG, "Failed to initialize OpenGLES 2.0 context");
        assert(0);
    }
    
    if (![EAGLContext setCurrentContext:_context]) {
        SC_DEBUG_ERROR_EX(kTAG, "Failed to set current OpenGL context");
        assert(0);
    }
}
...
- (void)render:(CADisplayLink*)displayLink {
    ...
    @synchronized(self) {
        if (((!_animating || _freezed) && !_stopping)) {
            return;
        }
...
-(void)touchesBegan:(NSSet *)touches withEvent:(UIEvent *)event {
    ...
    SC_DEBUG_INFO_EX(kTAG, "touchesBegan(%f, %f)", point.x, point.y);
    @synchronized(self) {
        [_annoCurrent release];
        _annoCurrent = nil;
        switch (_annoCurrentType) {
            case SCAnnotationTypeCircle: _annoCurrent = [[SCAnnotationCircle alloc] initWithView:self]; break;
            case SCAnnotationTypeArrow: _annoCurrent = [[SCAnnotationArrow alloc] initWithView:self]; break;
            case SCAnnotationTypeText:  _annoCurrent = [[SCAnnotationText alloc] initWithView:self]; break;
            case SCAnnotationTypeFreehand: _annoCurrent = [[SCAnnotationFreehand alloc] initWithView:self]; break;
            default: break;
        }
        if (_annoCurrent) {
            [_annoCurrent setDelegate:self];
            [_annoCurrent begin:&point];
        }
    }
}
-(void)touchesMoved:(NSSet *)touches withEvent:(UIEvent *)event {
    CGPoint point = [[touches anyObject] locationInView:self];
    SC_DEBUG_INFO_EX(kTAG, "touchesMoved(%f, %f)", point.x, point.y);
    @synchronized(self) {
        if (_annoCurrent) {
            [_annoCurrent move:&point];
        }
    }
}
...
-(void)touchesEnded:(NSSet *)touches withEvent:(UIEvent *)event {
    CGPoint point = [[touches anyObject] locationInView:self];
    SC_DEBUG_INFO_EX(kTAG, "touchesEnded(%f, %f)", point.x, point.y);
    @synchronized(self) {
        if (_annoCurrent) {
            [_annoCurrent end:&point];
        }
    }
}
...
- (void)touchesCancelled:(NSSet *)touches withEvent:(UIEvent *)event {
    CGPoint point = [[touches anyObject] locationInView:self];
    SC_DEBUG_INFO_EX(kTAG, "touchesCancelled(%f, %f)", point.x, point.y);
    @synchronized(self) {
        if (_annoCurrent) {
            [_annoCurrent cancel];
        }
...
-(BOOL) annotationReady:(id<SCAnnotation>)annotation {
    @synchronized(self) {
        if ([_delegate respondsToSelector:@selector(glviewAnnotationReady:)]) {
            [_delegate glviewAnnotationReady:annotation.json];
        }
    }
    return YES;
}
